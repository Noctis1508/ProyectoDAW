...
$pages = [];
$stmt = $conn->prepare("SELECT id, image_path, page_number, chapter_id FROM pages WHERE chapter_id = ? ORDER BY page_number ASC");
$stmt->bind_param("i", $chapter_id);
$stmt->execute();
$result = $stmt->get_result();
...
<div class="container py-5 reorder-wrapper">
    <h2 class="text-warning mb-3">Reordenar páginas del capítulo <?= htmlspecialchars($chapter_number) ?> - <?= htmlspecialchars($chapter_title) ?></h2>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <a href="edit_page.php?page_id=<?= $page['id'] ?>" class="btn btn-sm btn-outline-info"><span class="icon-pencil2" style="font-size: 1rem;"></span> Editar</a>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
    ...
    <a href="edit_chapter.php?id=<?= $page['chapter_id'] ?>" class="btn btn-secondary mt-4"><span class="icon-undo2" style="font-size: 1rem;"></span> Volver</a>
</div>
