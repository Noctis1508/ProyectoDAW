...
<div class="container py-5">
    ...
    <form action="" method="post" enctype="multipart/form-data" class="bg-secondary p-4 rounded mb-5">
        ...
        <button type="submit" class="btn btn-warning">
            <span class="icon-upload3" style="font-size: 1rem;"></span> Subir capítulo
        </button>
    </form>
    ...
    <?php else: ?>
        <div class="table-responsive">
            <table class="table table-dark table-striped align-middle text-nowrap">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Título</th>
                        <th class="d-none d-md-table-cell">Subido el</th>
                        <th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($capitulos as $cap): ?>
                        <tr>
                            ...
                            <td class="d-none d-md-table-cell"><?= date("d/m/Y", strtotime($cap['created_at'])) ?></td>
                            <td>
                                <div class="d-flex flex-wrap gap-1">
                                    <a href="edit_chapter.php?id=<?= $cap['id'] ?>" class="btn btn-sm btn-outline-warning">
                                        <span class="icon-pencil2" style="font-size: 0.85rem;"></span> Editar
                                    </a>
                                    <a href="delete_chapter.php?id=<?= $cap['id'] ?>&manga_id=<?= $manga_id ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('¿Estás seguro de eliminar este capítulo?')">
                                        <span class="icon-trash" style="font-size: 0.85rem;"></span>
                                    </a>
                                </div>
                            </td>
                        </tr>
